...
public class Realm extends Profile implements Serializable {
	...
	public LDAPConnectionDescriptor createSecondaryConnectionDescriptor()
			throws DirectoryException {
		final String followReferrals = getValue(
				"Directory.Secondary.FollowReferrals");
		final LDAPConnectionDescriptor secLcd = new LDAPConnectionDescriptor(
				"yes".equals(followReferrals)
		);
		...
		try {
			...
			boolean useSSL = "ldaps".equals(ldapUrl.getScheme());
			if (useSSL) {
				secLcd.setConnectionMethod(LDAPConnectionDescriptor.ConnectionMethod.SSL);
			}
			...
			if (-1 == ldapPort) {
				ldapPort = useSSL? DEFAULT_SECONDARY_SSL_LDAPPORT : DEFAULT_SECONDARY_LDAPPORT;
			}
