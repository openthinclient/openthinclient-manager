...
public class BaseCommand {
	...
	protected void init() {
		...
		Option op = new Option("i", "install-path", true,
		...
		getGlobal().addOption(op);
		op = new Option("c", "configuration", false,
				"force loading the server.xml (requires -i)");
		getGlobal().addOption(op);
	...
	public CommandLine getCommandLine(String command, String[] args) {
		...
		try {
			...
			System.err.println("Command line parsing failed for " + command
					+ ".  Reason: already selected " + ase.getMessage());
			System.exit(1);
		} catch (MissingArgumentException mae) {
			System.err.println("Command line parsing failed for " + command
					+ ".  Reason: missing argument " + mae.getMessage());
			System.exit(1);
		} catch (MissingOptionException moe) {
			System.err.println("Command line parsing failed for " + command
					+ ".  Reason: missing option " + moe.getMessage());
			System.exit(1);
		} catch (UnrecognizedOptionException uoe) {
			System.err.println("Command line parsing failed for " + command
					+ ".  Reason: unrecognized option" + uoe.getMessage());
			System.exit(1);
	...
	public void helpOnCommand(String command) {
		...
		if (getCommands().containsKey(command)) {
			...
			formatter.printHelp(getProductCommand() + " " + cmd + " [options]", cmd
					.getOptions());
		} else {
