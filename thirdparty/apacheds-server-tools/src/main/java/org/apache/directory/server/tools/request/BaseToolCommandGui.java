...
/**
 * Abstract class that must extended by every command that want to be used in a
 * program with a user interface
 * 
 */
public abstract class BaseToolCommandGui extends BaseToolCommand
		implements
			ToolCommandGui {
	...
	public void execute(Parameter[] params, ListenerParameter[] listeners)
		...
		toolCommandExecutorStub.execute((Parameter[]) parameters
				.toArray(new Parameter[0]), listeners);
	}
}
