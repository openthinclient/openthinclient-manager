...
public class ImportCommandCL extends BaseToolCommandCL {
	...
	public void processOptions(CommandLine cmd) {
		...
		// precedence
		{
			...
			parameters.add(new Parameter(ImportCommandExecutor.PORT_PARAMETER,
					new Integer(port)));
		}
		...
		if (cmd.hasOption('w')) {
			...
			parameters.add(new Parameter(ImportCommandExecutor.PASSWORD_PARAMETER,
					password));
		}
		...
		if (cmd.hasOption('a')) {
			...
			if (isDebugEnabled()) {
				System.out.println("authentication type overriden by -a option: "
						+ auth);
			}
		...
		if (cmd.hasOption('c')) {
			parameters.add(new Parameter(
					ImportCommandExecutor.CONFIGURATION_PARAMETER, new Boolean(true)));
		}

		// -------------------------------------------------------------------
		// Transform other options into params
		// -------------------------------------------------------------------
		parameters.add(new Parameter(ImportCommandExecutor.DEBUG_PARAMETER,
				new Boolean(isDebugEnabled())));
		parameters.add(new Parameter(ImportCommandExecutor.QUIET_PARAMETER,
				new Boolean(isQuietEnabled())));
		parameters.add(new Parameter(ImportCommandExecutor.VERBOSE_PARAMETER,
				new Boolean(isVerboseEnabled())));
	}
