...
public class ImportCommandExecutor extends BaseToolCommandExecutor {
	 ...
	 * 
	 * @throws UnknownHostException The hostname or the Address of server could
	 *           not be found
	...
	private LdapMessage readResponse(ByteBuffer bb) throws IOException,
		...
		while (true) {
			...
			if (nbRead == -1) {
				...
				if (ldapMessageContainer.getState() == TLVStateEnum.PDU_DECODED) {
					messageResp = ((LdapMessageContainer) ldapMessageContainer)
							.getLdapMessage();
					...
					if (messageResp instanceof BindResponse) {
						...
						if (resp.getLdapResult().getResultCode() != ResultCodeEnum.SUCCESS_VAL) {
							notifyOutputListener("Error : "
						...
						ExtendedResponse resp = ((LdapMessageContainer) ldapMessageContainer)
								.getLdapMessage().getExtendedResponse();
	...
	/**
	 * Send the entry to the encoder, then wait for a reponse from the LDAP server
	 * on the results of the operation.
	 * 
	 * @param entry The entry to add
	 * @param msgId message id number
	 */
	private int addEntry(Entry entry, int messageId) throws IOException,
			DecoderException, InvalidNameException, NamingException, EncoderException {
		...
		if (result.getResultCode() == ResultCodeEnum.SUCCESS_VAL) {
			if (isDebugEnabled()) {
				notifyOutputListener("Add of Entry " + entry.getDn()
						+ " was successful");
			}
	 ...
	 * @param msgId message id number
	 */
	private int deleteEntry(Entry entry, int messageId) throws IOException,
			DecoderException, InvalidNameException, NamingException, EncoderException {
		...
		if (result.getResultCode() == ResultCodeEnum.SUCCESS_VAL) {
			if (isDebugEnabled()) {
				notifyOutputListener("Delete of Entry " + entry.getDn()
						+ " was successful");
			}
	 ...
	 * @param msgId message id number
	 */
	private int changeModRDNEntry(Entry entry, int messageId) throws IOException,
			DecoderException, InvalidNameException, NamingException, EncoderException {
		...
		if (result.getResultCode() == ResultCodeEnum.SUCCESS_VAL) {
			if (isDebugEnabled()) {
				notifyOutputListener("ModifyDn of Entry " + entry.getDn()
						+ " was successful");
			}
	 ...
	 * @param msgId message id number
	 */
	private int changeModifyEntry(Entry entry, int messageId) throws IOException,
			DecoderException, InvalidNameException, NamingException, EncoderException {
		...
		while (modifications.hasNext()) {
			ModificationItemImpl modification = (ModificationItemImpl) modifications
					.next();
			...
			modifyRequest.addAttributeTypeAndValues(modification.getAttribute()
					.getID());

			for (NamingEnumeration values = modification.getAttribute().getAll(); values
					.hasMoreElements();) {
				Object value = values.nextElement();
		...
		if (result.getResultCode() == ResultCodeEnum.SUCCESS_VAL) {
			if (isDebugEnabled()) {
				notifyOutputListener("Modify of Entry " + entry.getDn()
						+ " was successful");
			}
	...
	/**
	 * Send the change operation to the encoder, then wait for a reponse from the
	 * LDAP server on the results of the operation.
	 * 
	 * @param entry The entry to add
	 * @param msgId message id number
	 */
	private int changeEntry(Entry entry, int messageId) throws IOException,
			DecoderException, InvalidNameException, NamingException, EncoderException {
	 ...
	 */
	private void bind(int messageId) throws EncoderException, DecoderException,
			IOException, ToolCommandException, NamingException {
		...
		if ("simple".equals(auth)) {
			...
			((SimpleAuthentication) authentication).setSimple(StringTools
	 ...
	 * @throws IOException
	 */
	private void unbind(int messageId) throws InvalidNameException,
			EncoderException, DecoderException, IOException {
	...
	private void processParameters(Parameter[] params) {
		...
		// Ignore-Errors param
		Boolean ignoreErrorsParam = (Boolean) parameters
				.get(IGNOREERRORS_PARAMETER);
	...
	private void execute() throws Exception {
		...
		if (ldifReader.containsEntries()) {
			...
			while (entries.hasNext()) {
				...
				// Check if we have had some error, has next() does not throw any
				// exception
			...
			notifyOutputListener(nbAdd + " users added in " + ((t1 - t0) / 1000)
					+ " seconds");
			...
			notifyOutputListener("Done!");
			notifyOutputListener(nbMod + " users changed in " + ((t1 - t0) / 1000)
					+ " seconds");
	 ...
	 * to the ldif file to import</li>
	 * <li>"IGNOREERRORS_PARAMETER" with a value of type 'Boolean', true to
	 * continue to process the file even if errors are encountered</li>
	 ...
	 * verbose</li>
	 * <li>"INSTALLPATH_PARAMETER" with a value of type 'String', representing
	 * the path to installation directory</li>
	 * <li>"CONFIGURATION_PARAMETER" with a value of type "Boolean", true to
	 * force loading the server.xml (requires "install-path")</li>
	 * </ul>
	 * <br />
	 * ListenersParameters :
	 * <ul>
	 ...
	 * <li>"ENTRYADDEDLISTENER_PARAMETER", a listener that will receive a message
	 * each time an entry is added. It returns the DN of the added entry as a
	 * String.</li>
	 * <li>"ENTRYADDFAILEDLISTENER_PARAMETER", a listener that will receive a
	 * message each time an entry has failed to import. It returns the DN og the
	 * entry on error.</li>
	 * </ul>
	 * <b>Note:</b> "HOST_PARAMETER", "PORT_PARAMETER", "USER_PARAMETER",
	 * "PASSWORD_PARAMETER", "AUTH_PARAMETER" and "IGNOREERRORS_PARAMETER" are
	 * required.
	 */
	 ...
	 * 
	 * @param listeners The provided Listeners
	 */
	private void processListeners(ListenerParameter[] listeners) {
		...
		ToolCommandListener entryAddedListener = (ToolCommandListener) parameters
				.get(ENTRYADDEDLISTENER_PARAMETER);
