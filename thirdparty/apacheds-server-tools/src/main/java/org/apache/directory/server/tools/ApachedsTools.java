...
public class ApachedsTools {
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			System.err.println("Type " + tools.getProductCommand()
					+ " help for usage.");
			System.exit(1);
		}
		...
		if ("help".equals(command)) {
			...
			System.out.println(tools.getProductCommand() + " version "
					+ tools.getProductVersion());
			System.exit(0);
		}

		BaseToolCommandCL cmd = (BaseToolCommandCL) tools.getCommands()
				.get(command);
		if (cmd == null) {
		...
		try {
			cmd.execute(cmdline,
					new ListenerParameter[]{
							new ListenerParameter(
									BaseToolCommandExecutor.OUTPUTLISTENER_PARAMETER,
									outputListener),
							new ListenerParameter(
									BaseToolCommandExecutor.ERRORLISTENER_PARAMETER,
									errorListener),
							new ListenerParameter(
									BaseToolCommandExecutor.EXCEPTIONLISTENER_PARAMETER,
									exceptionListener)});
		} catch (Exception e) {
			System.err.println("An error has occurred. Apache DS Tools must quit."
					+ "\nError: " + e.getMessage());
	...
	public static BaseCommand getInstance() throws InstantiationException,
		...
		String productUrl = props.getProperty("product.url",
				"http://directory.apache.org");
		String productDisplayName = props.getProperty("product.display.name",
				"Apache Directory Server");
		String productCommand = props.getProperty("product.command",
				"apacheds-tools");
		String productBanner = props.getProperty("product.banner",
				BaseCommand.BANNER);
		String productClass = props.getProperty("product.class",
				"org.apache.directory.server.tools.BaseCommand");

		BaseCommand baseCommand = (BaseCommand) Class.forName(productClass)
				.newInstance();
