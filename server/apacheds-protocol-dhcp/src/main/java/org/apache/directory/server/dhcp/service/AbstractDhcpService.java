...
public abstract class AbstractDhcpService implements DhcpService
{
     ...
     */
    public final DhcpMessage getReplyFor( InetSocketAddress localAddress, InetSocketAddress clientAddress,
        DhcpMessage request ) throws DhcpException
    {
        // ignore messages with an op != 1
        ...
        {
            System.out.println( "Missing message type option - plain BOOTP not supported." );
            return null;
        }
     ...
     * @param request
     *            the request message
     * @return response message or <code>null</code> to ignore (don't reply
     *         to) it.
     */
     ...
     *         to) it.
     */
    protected DhcpMessage handleINFORM( InetSocketAddress localAddress, InetSocketAddress clientAddress,
        DhcpMessage request ) throws DhcpException
    {
        if ( logger.isDebugEnabled() )
            logger.warn( "Got INFORM message: " + request + " from:  " + clientAddress );
        return null;
    }
    ...
    {
        if ( logger.isDebugEnabled() )
            logger.warn( "Got RELEASE message: " + request + " from:  " + clientAddress );
        return null;
    }
    ...
    {
        if ( logger.isDebugEnabled() )
            logger.warn( "Got REQUEST message: " + request + " from:  " + clientAddress );
        return null;
    }
    ...
    {
        if ( logger.isDebugEnabled() )
            logger.warn( "Got DISCOVER message: " + request + " from:  " + clientAddress );
        return null;
    }
     ...
     * <li>the server identifier set to the address of the interface the
     * request was received on
     * </ul>
     * 
     * @param localAddress
    ...
    /**
     * Determine address on which to base selection. If the relay agent address
     * is set, we use the relay agent's address, otherwise we use the address we
