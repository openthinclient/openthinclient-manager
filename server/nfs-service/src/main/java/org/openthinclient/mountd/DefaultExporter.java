...
public class DefaultExporter implements Exporter {
  ...
  public NFSExport getExport(InetAddress peer, String mountRequest) {
    ...
    for (int i = 0; i < roots.length; i++)
      if (roots[i].getAbsolutePath().toLowerCase().startsWith(mountRequest))
        return new NFSExport(roots[i].getAbsolutePath(), roots[i]);
  ...
  public List<NFSExport> getExports() {
    ...
    for (int i = 0; i < roots.length; i++)
      exports.add(new NFSExport(roots[i].getAbsolutePath(), roots[i]
          .getAbsoluteFile()));
