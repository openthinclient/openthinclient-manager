...
public class ConfigurationPanel implements WizardDescriptor.Panel {
  ...
  public boolean isValid() {
    ...
    if (!validate.hasErrors()) {
      if (validate.hasMessages()) {
        PropertyValidationMessage m = (PropertyValidationMessage) validate
            .getMessages().get(0);
  ...
  public void readSettings(Object settings) {
    ...
    Realm realm = (Realm) wd.getProperty("realm"); //$NON-NLS-1$

    Class type = (Class) wd.getProperty("type"); //$NON-NLS-1$
    ...
    try {
      ...
      if (newDirectoryObject instanceof Profile) {
        String schemaType = wd.getProperty("schemaType").toString(); //$NON-NLS-1$
      ...
      JComponent component = DirObjectEditor.getEditorForDirObject(
