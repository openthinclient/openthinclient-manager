 ...
 */
public class RealmConnectionErrorView extends AbstractDetailView {
	...
	public RealmConnectionErrorView(Exception exception) {
	...
	public JComponent getMainComponent() {
		...
		final DetailViewFormBuilder dfb = new DetailViewFormBuilder(new FormLayout(
				"f:p:g"), Messages.getBundle()); //$NON-NLS-1$

		final String message = exception.getMessage();
		final String source = exception.getCause().toString();

		final JLabel errorMessage = new JLabel(message);
		final JLabel sourceMessage = new JLabel(source);
		final JLabel loesung = new JLabel(Messages
				.getString("error.RealmConnectionErrorView.solutionstatement"));

		dfb
				.append(new CollapsibleTitlePanel(Messages
						.getString("error.RealmConnectionErrorView.server"), errorMessage,
						true));
		dfb.append(new CollapsibleTitlePanel(Messages
				.getString("error.RealmConnectionErrorView.message"), sourceMessage,
				true));
		dfb.append(new CollapsibleTitlePanel(Messages
				.getString("error.RealmConnectionErrorView.solution"), loesung, true));
