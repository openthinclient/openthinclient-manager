...
public class SetupADSACIPanel
  ...
  protected void updateComponentStates() {
    ...
    boolean enabled = baseCheckBox.isSelected();
  ...
  public boolean isEnabled(WizardDescriptor wd) {
    ...
    DirectoryType serverType = (DirectoryType) wd.getProperty("serverType"); //$NON-NLS-1$
    DirContext ctx = (DirContext) wd.getProperty("schema"); //$NON-NLS-1$

    if (null != serverType && serverType == DirectoryType.GENERIC_RFC)
      try {
        final boolean isADS = LDAPConnectionDescriptor.hasObjectClass(ctx,
            "apacheCatalogEntry"); //$NON-NLS-1$
